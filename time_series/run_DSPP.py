...
def main(seed=2024):
    ...
    class MultitaskDSPP(DSPP):
        ...
        def predict(self, test_x):
            with torch.no_grad():
                ...
                means, vars = preds.mean, preds.variance
                weights = self.quad_weights[:,None,None].exp().cpu()
            ...
            return (weights * means).sum(0), vars.mean(0), output
    
    num_tasks = train_y.size(-1)
    hidden_features = [2]
    num_quadrature_sites = 5 # 8
    ...
    lls = (model.likelihood.log_marginal(test_y, mdl_output) + model.quad_weights.unsqueeze(-1)).logsumexp(dim=0)
    NLL = -lls.mean().item()
    ...
    f_name = os.path.join('./results/ts_DSPP_'+str(model.num_layers)+'layers.txt')
    ...
    # # plt.show()
    # os.makedirs('./results', exist_ok=True)
    # plt.savefig('./results/ts_DSPP_'+str(model.num_layers)+'layers.png',bbox_inches='tight')
