...
def main(seed=2024):
    ...
    model = clsDeepGP(input_dim=input_dim, num_classes=num_classes)
    ...
    # np.savetxt(os.path.join('./results/mnist_DGP.txt'),stats,fmt="%s",delimiter=',',header=','.join(header))
    ...
    header = ['Method', 'ACC', 'STD', 'NLL', 'time']
    np.savetxt(os.path.join('./results',args.dataset_name+'_DGP.txt'),stats,fmt="%s",delimiter=',',header=','.join(header))
    np.savez_compressed(os.path.join('./results',args.dataset_name+'_DGP'), loss=np.stack(loss_list), acc=np.stack(acc_list), std=np.stack(std_list), nll=np.stack(nll_list), times=times)
    ...
    plt.savefig(os.path.join('./results',args.dataset_name+'_DGP.png'), bbox_inches='tight')
