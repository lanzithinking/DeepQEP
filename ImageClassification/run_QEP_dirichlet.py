...
from gpytorch.likelihoods import QExponentialDirichletClassificationLikelihood
...
def main(seed=2024):
    ...
    likelihood = QExponentialDirichletClassificationLikelihood(torch.tensor(getattr(train_loader.dataset,{'mnist':'train_labels','cifar10':'targets'}[args.dataset_name])), learn_additional_noise=True)
    ...
    def train(epoch):
        ...
        for data, target in minibatch_iter:
            ...
            loss = -mll(output, likelihood._prepare_targets(target,num_classes=model.likelihood.num_classes)[1].T).sum()
            loss.backward()
            optimizer.step()
    ...
    def test(epoch):
        ...
        with torch.no_grad(), gpytorch.settings.fast_pred_var():
            for data, target in test_loader:
                ...
                ll = model.likelihood.log_marginal(target, appx_dist).mean(0).cpu()
