"Deep Kernel Learning Q-Exponential Process Classification Model"
...
def main(seed=2024):
    ...
    class QExponentialProcessLayer(ApproximateQEP):
        def __init__(self, input_dims, output_dims, grid_bounds=(-10., 10.), grid_size=128, mean_type='constant'):
            ...
            variational_distribution = CholeskyVariationalDistribution(
                num_inducing_points=grid_size, batch_shape=self.batch_shape, power=self.power
            )
    ...
    class DKLQEP(gpytorch.Module):
        def __init__(self, feature_extractor, output_dims, grid_bounds=(-10., 10.), likelihood=None):
            super(DKLQEP, self).__init__()
            self.feature_extractor = feature_extractor
            self.qep_layer = QExponentialProcessLayer(input_dims=feature_extractor.output_dims, output_dims=output_dims, grid_bounds=grid_bounds)
            self.output_dims=output_dims
    ...
    model = DKLQEP(feature_extractor, num_classes, likelihood=likelihood)
    # set device
    model = model.to(device)
    # likelihood = likelihood.to(device)
    
    # "Loss" for QEPs - the marginal log likelihood
    mll = VariationalELBO(likelihood, model.qep_layer, num_data=len(train_loader.dataset))
    ...
    #     {'params': model.qep_layer.hyperparameters(), 'lr': lr * 0.1},
    ...
    def test(epoch):
        ...
        with torch.no_grad(), gpytorch.settings.num_likelihood_samples(16):
            for data, target in test_loader:
                ...
                ll = model.likelihood.log_marginal(target, appx_dist).cpu()
    ...
    np.savez_compressed(os.path.join('./results',args.dataset_name+'_DKLQEP_'+str(model.feature_extractor.num_layers)+'layers'), loss=np.stack(loss_list), acc=np.stack(acc_list), auc=np.stack(auc_list), nll=np.stack(nll_list), times=times)
