...
def main(seed=2024):
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        torch.save({'model': state_dict, 'likelihood': likelihood_state_dict}, os.path.join('./results','dklgp_'+args.dataset_name+'_checkpoint.dat'))
    ...
    np.savetxt(os.path.join('./results',args.dataset_name+'_DKLGP.txt'),stats,fmt="%s",delimiter=',',header=','.join(header))
    np.savez_compressed(os.path.join('./results',args.dataset_name+'_DKLGP'), loss=np.stack(loss_list), acc=np.stack(acc_list), std=np.stack(std_list), nll=np.stack(nll_list), times=times)
    ...
    plt.savefig(os.path.join('./results',args.dataset_name+'_DKLGP.png'), bbox_inches='tight')
