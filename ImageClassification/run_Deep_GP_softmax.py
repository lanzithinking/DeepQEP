...
def main(seed=2024):
    ...
    model = clsDeepGP(in_features=input_dim, out_features=num_classes, hidden_features=hidden_features, likelihood=likelihood, num_inducing=num_inducing)
    # set device
    ...
    # Train the model
    os.makedirs('./results', exist_ok=True)
    f_name = args.dataset_name+'_DGP_softmax_'+str(model.num_layers)+'layers'
    loss_list = []
    acc_list = []
    auc_list = []
    nll_list = []
