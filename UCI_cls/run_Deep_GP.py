...
def main(seed=2024):
        ...
        def predict(self, test_loader):
            ...
            with torch.no_grad():
                for x_batch, y_batch in test_loader:
                    ...
                    lls.append(model.likelihood.log_marginal(batch_targets, test_dist).mean(0).cpu())
    ...
    for i in epochs_iter:
        ...
        for x_batch, y_batch in minibatch_iter:
            ...
            #         output.mean.mean(0).argmax(-1).eq(y_batch).mean(dtype=float).item()
            #     ))
            optimizer.step()
            loss_i += loss.item()
    ...
    f_name = os.path.join('./results',args.dataset_name+'_DGP_'+str(model.num_layers)+'layers.txt')
