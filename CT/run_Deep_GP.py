...
def main(seed=2024):
    ...
    model = MultitaskDeepGP(n, latent_dim, num_tasks, hidden_features, latent_init)
    
    # training
    model.train()
    optimizer = torch.optim.Adam(model.parameters(), lr=0.1)
    mll = DeepApproximateMLL(VariationalELBO(model.likelihood, model, num_data=n, beta=100))
    ...
    filename = 'CT_proj'+str(args.n_angles)+'_DGP_'+str(model.num_layers)+'layers'
