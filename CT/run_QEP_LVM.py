...
import torch
import tqdm#.notebook import trange
...
def main(seed=2024):
    ...
    class bQEPLVM(BayesianQEPLVM):
        def __init__(self, n, data_dim, latent_dim, n_inducing, lsqr=False):
            ...
            prior_x = QExponentialPrior(X_prior_mean, torch.ones_like(X_prior_mean)*1000, power=self.power)
    ...
    # Likelihood
    likelihood = QExponentialLikelihood(batch_shape=model.batch_shape, power=POWER, miu=False)
    
    # Declaring the objective to be optimised along with optimiser
    # (see models/latent_variable.py for how the additional loss terms are accounted for)
    mll = VariationalELBO(likelihood, model, num_data=N, beta=1000)
    ...
    filename = 'CT_proj'+str(args.n_angles)+'_QEP-LVM'
    ...
    stats = np.array([seed,'QEP-LVM']+[np.array2string(r, precision=4) for r in stats])[None,:]
