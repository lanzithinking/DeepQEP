...
def main(seed=2024):
    ...
    model = clsDeepGP(in_features=train_x.shape[-1], out_features=likelihood.num_classes, hidden_features=hidden_features, likelihood=likelihood)
    ...
    f_name = os.path.join('./results_'+dataset,'cls_DeepGP.txt')
    ...
    # plt.savefig('./results_'+dataset+'/cls_logits_DeepGP_'+str(model.num_layers)+'layers.png',bbox_inches='tight')
    #
    #
    # # boundaries
    # fig = plt.figure(figsize=(5, 5))
    # plt.contourf(test_x_mat.numpy(), test_y_mat.numpy(), pred_means.max(1)[1].reshape((n_test,n_test)))
    # plt.title('Deep GP', fontsize=20)
    # plt.savefig('./results_'+dataset+'/cls_boundaries_DeepGP_'+str(model.num_layers)+'layers.png',bbox_inches='tight')
