...
def main(seed=2024):
    ...
    class QEPModel(ExactQEP):
        def __init__(self, train_x, train_y, likelihood, num_classes):
            super(QEPModel, self).__init__(train_x, train_y, likelihood)
    ...
    likelihood = QExponentialDirichletClassificationLikelihood(train_y, learn_additional_noise=True, power=torch.tensor(POWER), miu=False)
    model = QEPModel(train_x, likelihood.transformed_targets, likelihood, num_classes=likelihood.num_classes)
    ...
    f_name = os.path.join('./results_'+dataset,'cls_QEP.txt')
    ...
    # plt.savefig('./results_'+dataset+'/cls_QEP_logits.png',bbox_inches='tight')
    #
    # # boundaries
    # fig = plt.figure(figsize=(5, 5))
    # plt.contourf(test_x_mat.numpy(), test_y_mat.numpy(), pred_means.max(0)[1].reshape((n_test,n_test)))
    # plt.title('QEP', fontsize=20)
    # plt.savefig('./results_'+dataset+'/cls_QEP_boundaries.png',bbox_inches='tight')
