...
def main(seed=2024):
    ...
    class DSPP_Layer(DSPPLayer):
        def __init__(self, input_dims, output_dims, num_inducing=64, mean_type='constant', Q=8):
    ...
    model = clsDSPP(in_features=train_x.shape[-1], out_features=likelihood.num_classes, hidden_features=hidden_features, likelihood=likelihood)
    ...
    with gpytorch.settings.fast_pred_var(), torch.no_grad():
        ...
        pred_means = (test_dist.mean+model.quad_weights[:,None,None]).mean(0)
        ...
        lls = (model.likelihood.log_marginal(test_targets, test_dist) + model.quad_weights.unsqueeze(-1)).logsumexp(dim=0)
    ...
    f_name = os.path.join('./results_'+dataset,'cls_DSPP.txt')
    ...
    # plt.savefig('./results_'+dataset+'/cls_logits_DSPP_'+str(model.num_layers)+'layers.png',bbox_inches='tight')
    #
    #
    # # boundaries
    # fig = plt.figure(figsize=(5, 5))
    ...
    # plt.savefig('./results_'+dataset+'/cls_boundaries_DSPP_'+str(model.num_layers)+'layers.png',bbox_inches='tight')
